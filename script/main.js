...
clickableName.addEventListener('click', function() {
    ...
    // 2. Play music (if not already playing)
    ...
    // 3. Cute "Pop" Animation using GSAP
    gsap.to("#clickableName", {
        scale: 1.3,
        duration: 0.2,
        yoyo: true,
        repeat: 1,
    ...
    // 4. Optional: Create a mini burst of hearts right at the click location
    for(let i = 0; i < 10; i++) {
        const burstHeart = new Heart();
        burstHeart.x = window.innerWidth / 2;
        burstHeart.y = window.innerHeight / 2.5; // Near the text
        burstHeart.speed = Math.random() * 5 + 2;
        hearts.push(burstHeart);
    }
    
    // You can also trigger your GSAP timeline for the rest of the screens here!
});    
...
function createHeart() {
    ...
    gsap.to(heart, {
        ...
        ease: "none",
        onComplete: () => heart.remove() // Clean up DOM
    });
}

// Start generating hearts every 400ms
